...
app.whenReady().then(async () => {
  ...
  try {
    ...
    console.error('Auto-updater failed:', updateError);
  }

// 🛑 TOOLBAR: force-hide the File / Edit / View / Window / Help menu bar.
// To re-enable later, delete or comment out this whole block.
try {
  const { Menu } = require('electron');
  // Remove any app menu (prevents Electron’s default menu on Win/Linux)
  Menu.setApplicationMenu(null);

  // Also hide the window’s menubar and keep it hidden (even on Alt key)
  if (mainWindow) {
  ...
  console.warn('Toolbar hide block failed:', e?.message || e);
}
  ...
  // 🚫 Toolbar disabled — comment out this line to hide File/Edit/View/Help menu
// try { buildMenu(); } catch {}
// ✅ IPC Handlers
  ...
  ipcMain.handle('getUniqueSubCategories', async (_event, arg) => {
    const category = (arg && typeof arg === 'object') ? arg.category : arg;
